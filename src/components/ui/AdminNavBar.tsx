...
const AdminNavBar = ({ onMenuClick }: AdminNavBarProps) => {
  ...
  const heading = useMemo(() => {
    ...
    if (p.startsWith("/admin/cases")) return "All Cases";
    if (p.startsWith("/admin/user-management")) return "User Management";
    if (p.startsWith("/admin/supervision-management"))
    ...
    <header className="bg-white border-border/60 border-b">
      <div className="flex justify-between items-center mx-auto px-3 sm:px-4 md:px-6 py-3 md:py-4 w-full">
        <div className="flex flex-1 items-center gap-3 sm:gap-4 min-w-0">
          <button
            onClick={onMenuClick}
            className="lg:hidden flex justify-center items-center hover:bg-gray-100 p-2 rounded-md"
        ...
        <div className="relative ml-2 sm:ml-4 shrink-0" ref={ref}>
          <button
            onClick={() => setOpen((v) => !v)}
            className="flex items-center gap-2 sm:gap-3 hover:bg-gray-100 px-2 sm:px-3 py-1 rounded-md"
          >
            <div className="flex justify-center items-center bg-gray-200 rounded-full w-8 sm:w-9 h-8 sm:h-9 text-gray-700 text-sm uppercase">
          ...
          {open && (
            <div className="right-0 z-20 absolute bg-white shadow-lg mt-2 border rounded-md w-36 sm:w-44">
              <Link
                to="/admin/settings"
                className="block hover:bg-gray-100 px-3 sm:px-4 py-2 text-gray-700 text-sm"
              >
                Settings
              </Link>
              <button
                onClick={handleLogout}
                className="hover:bg-gray-50 px-3 sm:px-4 py-2 w-full text-red-600 text-sm text-left"
              >
                Logout
              </button>
            </div>
